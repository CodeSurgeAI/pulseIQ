...
export function UserSettingsModal({ isOpen, onClose }: UserSettingsModalProps) {
  ...
  const getModulesForRole = () => {
    ...
    switch (user.role) {
      case 'admin':
        // Admin doesn't have pages for these modules in navigation
        return allModules.filter(module => 
          !['kpiForm', 'recommendations', 'leaderboard', 'alerts'].includes(module)
        );
      case 'director':
        return allModules.filter(module => 
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {availableModules.map((module) => {
                    ...
                    // Map settings module to config screen key
                    const moduleToConfigKey: Record<keyof UserSettings['dashboardModules'], string> = {
                      users: 'users',
                      hospitals: 'hospitals',
                      clinicalAI: 'clinicalDecisionSupport',
                      workforceManagement: 'workforceManagement',
                      supplyChain: 'supplyChainIntelligence',
                      patientFlow: 'patientFlowManagement',
                    ...
                    const configKey = moduleToConfigKey[module];
                    const isConfigEnabled = configService.isScreenEnabled(user.role as any, configKey);
                    ...
                    return (
                      <Card 
                        ...
                        } ${!isConfigEnabled ? 'opacity-60' : 'cursor-pointer'}`}
                        onClick={() => {
