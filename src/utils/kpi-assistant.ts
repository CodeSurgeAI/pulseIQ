...
export function answerLeadershipQuestion(question: string, role?: string): string {
  ...
  if (q.includes('readmission') || q.includes('re-admission')) {
    return `${preface}our current readmission rate is ${readmit?.currentValue}${readmit?.unit ?? ''} vs target ${readmit?.targetValue}${readmit?.unit ?? ''}. To reduce this, prioritize a 48-hour post-discharge call for high-risk patients, schedule follow-ups before discharge, and provide simplified medication reconciliation at bedside. These steps typically lower readmissions by 20–30% within 6–8 weeks.`;
  }
  if (q.includes('wait') || q.includes('throughput') || q.includes('flow')) {
    return `${preface}average wait time is ${waitTime?.currentValue} ${waitTime?.unit}. Implement fast-track triage for low-acuity cases and align staffing to peak arrival hours. Also, enable auto bed-turnover alerts once discharge orders are signed to cut boarding time.`;
  }
  if (q.includes('occupancy') || q.includes('beds')) {
    return `${preface}bed occupancy is ${occupancy?.currentValue}${occupancy?.unit ?? '%'} with a target of ${occupancy?.targetValue}${occupancy?.unit ?? '%'}.
Optimizing discharge-before-noon and predictive bed allocation can raise effective capacity by 3–6% without adding physical beds.`;
  ...
  if (q.includes('satisfaction') || q.includes('experience') || q.includes('hca')) {
    return `${preface}patient satisfaction is ${satis?.currentValue} (target ${satis?.targetValue}). Quick wins: bedside rounding at shift changes, proactive communication about delays, and post-visit SMS feedback loops with rapid service recovery for low ratings.`;
  }
...
// Intent detection for voice/text commands that map to in-app actions
...
export function detectIntent(utterance: string): AssistantAction {
  const u = utterance.toLowerCase();

  // Notifications
  if (u.includes('mark') && u.includes('notification') && (u.includes('read') || u.includes('as read'))) {
    return { type: 'markAllNotifications' };
  }

  // Navigation intents
  if (u.includes('go to') || u.includes('open') || u.includes('navigate')) {
    if (u.includes('dashboard')) return { type: 'navigate', path: '/dashboard' };
    if (u.includes('alerts')) return { type: 'navigate', path: '/director/alerts' };
    if (u.includes('leaderboard')) return { type: 'navigate', path: '/director/leaderboard' };
    if (u.includes('financial')) return { type: 'navigate', path: '/director/financial-performance' };
    if (u.includes('patient flow')) return { type: 'navigate', path: '/director/patient-flow-management' };
    if (u.includes('supply chain')) return { type: 'navigate', path: '/director/supply-chain-intelligence' };
    if (u.includes('workforce')) return { type: 'navigate', path: '/director/workforce-management' };
    if (u.includes('kpi') && (u.includes('submit') || u.includes('form') || u.includes('entry'))) return { type: 'navigate', path: '/manager/kpi-form' };
    if (u.includes('setting')) return { type: 'openSettings' };
    // Specific insights
    if (u.includes('readmission') && (u.includes('detail') || u.includes('cardiology'))) {
      return { type: 'insight', content: generateReadmissionInsight('cardiology') };
    }
    if (u.includes('recommendation')) {
      return { type: 'insight', content: generateRecommendationsInsight() };
    }
    if (u.includes('anomal') || u.includes('outlier')) {
      return { type: 'insight', content: generateAnomaliesInsight() };
    }
  }

  // Settings without navigation keywords
  if (u.includes('setting') || u.includes('preferences')) return { type: 'openSettings' };

  return { type: 'none' };
}

function generateReadmissionInsight(dept?: string): string {
  const { metrics, anomalies } = getAssistantContext();
  const readmit = pickMetric(metrics, 'Readmission');
  const cardioAnom = anomalies.find(a => a.kpiName.toLowerCase().includes('readmission') && a.departmentId === '2');
  const deptLabel = dept ? dept[0].toUpperCase() + dept.slice(1) : 'target department';
  const parts: string[] = [];
  parts.push(`${deptLabel} readmission overview:`);
  parts.push(`• Current rate: ${readmit?.currentValue}${readmit?.unit ?? '%'} vs target ${readmit?.targetValue}${readmit?.unit ?? '%'}.`);
  if (cardioAnom) {
    parts.push(`• Recent anomaly: ${cardioAnom.detectedValue}% (expected ${cardioAnom.expectedRange.min}–${cardioAnom.expectedRange.max}%). Severity ${cardioAnom.severity}.`);
    parts.push(`• Likely factors: ${cardioAnom.possibleCauses.slice(0, 3).join(', ')}.`);
  }
  parts.push('• Actions: 48-hour post-discharge calls for high-risk patients; schedule follow-ups before discharge; simplify bedside med reconciliation.');
  return parts.join('\n');
}

function generateRecommendationsInsight(): string {
  const { recommendations } = getAssistantContext();
  const top = recommendations
    .filter(r => r.status === 'pending' || r.status === 'in_progress')
    .slice(0, 2)
    .map(r => `• ${r.title} (${r.priority}): ${r.description}`)
    .join('\n');
  return top || 'No active recommendations at this time.';
}

function generateAnomaliesInsight(): string {
  const { anomalies } = getAssistantContext();
  const latest = anomalies.slice(0, 2).map(a => `• ${a.kpiName}: detected ${a.detectedValue} (expected ${a.expectedRange.min}-${a.expectedRange.max}), severity ${a.severity}`);
  return latest.length ? latest.join('\n') : 'No anomalies detected recently.';
}
